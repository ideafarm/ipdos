...
int main( int cArgP , char* ppostArgP[] , char* ppostEnvP[] )
{
...

    {
        ...
        if( !oshManager )
        {
            ...
            logTimeGF( "IPDOS (tm) Service / could not open services manager [rc]:   " ) ;
            logGF( rc ) ;
            logGF( "\r\n" ) ;
        }
        else
        {
            ...
            {
                unsigned rc = GetLastError() ;
                logTimeGF( "IPDOS (tm) Service / could not open the \"IPDOS (tm)\" service [rc]:   " ) ;
                logGF( rc ) ;
                logGF( "\r\n" ) ;
            }
            else if( bStop )
            {
                ...
                info.dwReason = SERVICE_STOP_REASON_FLAG_UNPLANNED | SERVICE_STOP_REASON_MAJOR_OPERATINGSYSTEM | SERVICE_STOP_REASON_MINOR_NONE ;
                info.pszComment = /*(LPSTR)*/"MS Windows sucks, so I had to manually send the \"!stop\" control." ;

                unsigned bOk = ControlServiceExA( oshService , SERVICE_CONTROL_STOP , SERVICE_CONTROL_STATUS_REASON_INFO , &info ) ;
                if( !bOk )
                {
                    unsigned rc = GetLastError() ;
                    logTimeGF( "IPDOS (tm) Service / could not send the STOP control value [rc]:   " ) ;
                    logGF( rc ) ;
                    logGF( "\r\n" ) ;
                }
                else
                {
                    logTimeGF( "IPDOS (tm) Service / sent the STOP control value\r\n" ) ;
                }
            }
            else
            {
                ...
                {
                    unsigned rc = GetLastError() ;
                    logTimeGF( "IPDOS (tm) Service / could not change preshutdown timeout value [rc]:   " ) ;
                    logGF( rc ) ;
                    logGF( "\r\n" ) ;
                }
                else
                {
                    logTimeGF( "IPDOS (tm) Service / preshutdown timeout value changed to [minutes]:   " ) ;
